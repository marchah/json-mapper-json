...
function getValue(from, settings, root) {
  ...
  for (var i = 0; i !== settings.path.length; i++) {
    if (_.has(value, settings.path[i]) || (_.isObject(value) && !_.isUndefined(_.get(value, settings.path[i])))) {
...
function parseProperties(from, properties, root) {
    ...
    for (var i = 0; i !== from.length; i++) {
      ...
      for (var fieldName in template) {
        if (template.hasOwnProperty(fieldName)) {
          ret = parseProperties(from[i], template[fieldName], root);
